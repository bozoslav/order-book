#!/usr/bin/env python3
"""
Visualization script for Google Benchmark results.
Parses JSON output and generates graphs for latency distributions and throughput.
"""
...
def extract_latency_benchmarks(data: Dict[str, Any]) -> List[Dict[str, Any]]:
    """Extract benchmarks related to latency measurements."""
    latency_benchmarks = []
...
def extract_throughput_benchmarks(data: Dict[str, Any]) -> List[Dict[str, Any]]:
    """Extract benchmarks related to throughput measurements."""
...
def calculate_percentiles(times: List[float]) -> Dict[str, float]:
    """Calculate p50, p99, p99.9 percentiles from time measurements."""
    if not times:
...
def plot_latency_comparison(latency_benchmarks: List[Dict[str, Any]], output_dir: Path):
    """Create bar chart comparing latencies across different operations."""
    if not latency_benchmarks:
        print("No latency benchmarks found")
        return
    
    # Group by base benchmark name
    benchmark_groups = {}
    for bench in latency_benchmarks:
        name = bench.get('name', '')
        # Extract base name (remove parameters)
    ...
    # Plot 2: Percentile comparison for latency distribution benchmarks
    percentile_data = {}
    for bench in latency_benchmarks:
        name = bench.get('name', '')
...
def plot_throughput_comparison(throughput_benchmarks: List[Dict[str, Any]], output_dir: Path):
    """Create bar chart comparing throughput across different scenarios."""
    if not throughput_benchmarks:
        print("No throughput benchmarks found")
        return
    
    # Group by base name and range parameter
    benchmark_data = []
    ...
    for idx, (name, items) in enumerate(sorted(grouped_data.items())[:3]):
        ...
        if len(items_sorted) > 1:
            ...
            # Single bar for single measurement
...
def plot_scaling_analysis(latency_benchmarks: List[Dict[str, Any]], output_dir: Path):
    """Analyze how operations scale with increasing book size."""
    scaling_benchmarks = []
    for bench in latency_benchmarks:
        name = bench.get('name', '')
        if '/' in name:  # Has range parameter
