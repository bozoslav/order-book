...
void OrderBook::addOrder(int id, Price price, int quantity, bool isBuy,
                         long long userId, orderType type,
  ...
  if (type == orderType::FOK) {
    ...
    if (isBuy) {
      ...
      if (maxIdx >= 0) {
        ...
        int startLevel =
            (bestAskIndex != -1) ? bestAskIndex : asks.findNextNonEmptyFrom(0);
      ...
      if (minIdx < MAX_LEVELS) {
        ...
        int startLevel = (bestBidIndex != -1)
                             ? bestBidIndex
                             : bids.findPrevNonEmptyFrom(MAX_LEVELS - 1);
        int level = startLevel;
        ...
        while (currIdx != -1 && remainingQty > 0) {
          ...
          if (bookOrder.quantity == 0) {
            ...
            if (bids.isEmptyLevel(level) && bestBidIndex == level) {
              int prev =
                  (level == 0) ? -1 : bids.findPrevNonEmptyFrom(level - 1);
              bestBidIndex = prev;
            }
          }
          currIdx = nextIdx;
...
void OrderBook::modifyOrder(int id, Price newPrice, int newQuantity,
                            std::vector<Trade>& trades) {
...
void OrderBook::printOrderBook() const {
  std::cout << "\nBIDS (price desc):\n";
  int level = (bestBidIndex != -1) ? bestBidIndex
                                   : bids.findPrevNonEmptyFrom(MAX_LEVELS - 1);
  while (level != -1) {
    ...
    while (curr != -1) {
      ...
      std::cout << "ID: " << node.order.id << ", Price: " << p.value
...
int OrderBook::numOrders() const { return globalPool.currSize(); }